package com.buuchezo.apigateway.exceptions;
...
public class GlobalExceptionHandler implements ErrorWebExceptionHandler {
    // converts errors into json Objects
    private final ObjectMapper objectMapper = new ObjectMapper();

    @Override
    public Mono<Void> handle(ServerWebExchange exchange, Throwable ex) {
        // if the error has already been sent to the client we don´t need to do anything
        ...
        return Mono.fromCallable(() -> objectMapper.writeValueAsBytes(errorBody))
                .map(bytes ->
                        exchange.getResponse().bufferFactory()
                                .wrap(bytes))
                .flatMap(dataBuffer -> exchange
                        .getResponse().writeWith(Mono.just(dataBuffer)))
